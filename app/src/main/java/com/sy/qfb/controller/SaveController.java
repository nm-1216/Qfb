...
public class SaveController extends BaseController{
    public static final int SAVE_DATA_KEY = 2001;

    public void saveData(BaseController.UpdateViewAsyncCallback<Boolean> viewCallback, List<MeasureData> lstData)
    {
        doAsyncTask(SAVE_DATA_KEY, viewCallback,
                new DoAsyncTaskCallback<List<MeasureData>, Boolean>() {
                    @Override
                    public Boolean doAsyncTask(List<MeasureData>... lists) throws IException {
                        saveDataImpl(lists[0]);
                        return true;
                    }
                }, lstData);
    }

    private void saveDataImpl(List<MeasureData> lstData) {
